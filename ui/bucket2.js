...
function addCont(){
    var doc = create_cont();
    console.log(UI.session.save(doc));
    console.log(doc);
    var cont = new Cont(UI.session,doc);
    UI.bucket.appendChild(cont.element);
    UI.sequence.do_reorder();    
}

//TODO:HERE
var Cont = function(session,doc){
    session.subscribe(doc._id, this.on_change, this);
    UI.conts.push(this);
    this.doc = doc;
    this.id = doc._id;
    this.session=session;
    this.element = $el('div', {'class': 'cont','id': doc._id});    
    this.list = $el('ul', {'style': 'display = "none"'});
    this.comp = $el('div', {'class': 'comp'})
    this.element.appendChild(this.list);
    this.element.appendChild(this.comp);
    this.element.onmousedown = $bind(this.on_mousedown, this);
    this.element.ondblclick = $bind(this.on_dblclick, this);
    this.element.onclick = $bind(this.on_click, this);
    this.expanded = true;
    this.over = null;
    this.x = 150;
    this.y = 150;
...
Cont.prototype = {
    ...
    get x() {
        if(this.element)return parseInt(this.element.style.left);
        else return -1;
    },

    get y() {
        if(this.element)return parseInt(this.element.style.top);
        else return -1;
    },
    ...
    save: function(){
        this.doc.node.src = Array();
        for(var a in this.list.childNodes){
            if(this.list.childNodes[a].id)this.doc.node.src.push(this.list.childNodes[a].id);
        }
        this.session.save(this.doc,true);
    },

    refresh: function(){
        this.comp.innerHTML = '<div class="top">' + this.list.firstChild.firstChild.firstChild.outerHTML + '</div>';
        this.comp.innerHTML += '<div class="bot">'+ this.list.lastChild.lastChild.firstChild.outerHTML + '</div>';
    },
    ...
    close: function(){
        ...
        console.log("close");
        this.comp.innerHTML = "";
        var len = this.list.childNodes.length;
        console.log(len);       
        if(len){
            this.refresh();
        }else{
            if(len == 1){
                console.log("save frist child");
                var el = this.list.childNodes[0];
                el.style.top = this.y + 'px';
                el.style.left = this.x + 'px';
                el.style.marginLeft = '0px';
                el.style.marginRight = '0px';                
                $unparent(el); 
                UI.bucket.appendChild(el);
            }
            UI.bucket.removeChild(this.element);
            UI.sequence.do_reorder();
            for(var a in UI.conts)if(UI.conts[a].id == this.id)UI.conts.splice(a,1);
            return;
        }
    ...
    reset_margin: function(){
        for(var a = 0;a<this.list.childNodes.length;a++){
            this.list.childNodes[a].style.marginLeft = 0 + 'px';
            this.list.childNodes[a].style.marginRight = 0 + 'px';
        }
    },
    slice_on: function(x){
        ...
        this.reset_margin();
        this.pos = parseInt(x/130);
        if(this.pos >= this.list.childNodes.length) this.list.lastChild.style.marginRight = 130 + 'px';
        else if(this.list.childNodes[this.pos].classList[1] != 'selected') this.list.childNodes[this.pos].style.marginLeft = 130 + 'px';
    },
 ...
 on_drop: function(dnd) {
    ...
    on_change: function(doc) {
        ...
        this.doc = doc;
        for(var a in doc.node.src){
            console.log(doc.node.src[a]);
            var tdoc = UI.session.get_doc(doc.node.src[a]);
            var slice = new Slice(UI.session, tdoc);
            slice.start.request_thumbnail();
            slice.end.request_thumbnail();
            $unparent(slice.element);
            this.list.appendChild(slice.element);
    }
    },
}
...
Slice.prototype = {
    ...
    on_drag: function(dnd) {
        ...
        var f = 0.65;
        //if(this.oncont)UI.conts[oncontn].reset_margin();
        this.oncont = false;
        for(var a in UI.conts){
            var b = UI.conts[a];
            if(b.element && b.x < x && b.y < y && b.x+parseInt(b.element.offsetWidth) > x && b.y+parseInt(b.element.offsetHeight) > y){
                b.expand();
                this.oncont = true;
                this.oncontn = a;
                }
        ...
        if(this.oncont) UI.conts[this.oncontn].slice_on(x-UI.conts[this.oncontn].x);   
    ...
    on_drop: function(dnd) {
        ...
        if(this.oncont){
                this.x = null;
                this.y = null;
                var pos = UI.conts[this.contn].pos;
                console.log(pos);
                $unparent(this.element);
                $unselect(UI.selected);
                if(pos == -1) UI.conts[this.contn].list.appendChild(this.element);
                else UI.conts[this.contn].list.insertBefore(this.element,UI.conts[this.contn].list.childNodes[pos]);
                //UI.conts[this.contn].list.appendChild(this.element);
                //UI.conts[this.contn].doc.node.src.push(this.id);
                //this.session.save( UI.conts[this.contn].doc,true);
                UI.conts[this.contn].reset_margin();
                UI.conts[this.contn].save();
                UI.sequence.do_reorder();
                for(var a in UI.slices)if(UI.slices[a].id == this.id)UI.slices.splice(a,1);
                return;            
        }
        UI.sequence.reset();
        if (this.inbucket) {
        ...
        if(UI.conts[this.contn]) UI.conts[this.contn].save();
        UI.sequence.do_reorder();
        if (UI.player.active) {
            UI.player.resume();
        }
...
function closeAll(){
    for(var a in UI.conts)UI.conts[a].close();
}


function unselectAll(){
        for(var a in UI.selected){
            var b = UI.selected[a];
            $unselect(b);
        }
        UI.selected = Array();
...
var selector = {
    ...
    on_mouse_down: function(ev){
        ...
        unselectAll();
        selector.down = true;
        selector.el = $el('div', {'class': 'selector','id': 'sel'});
        selector.stx = ev.x;
        selector.sty = ev.y;
        selector.el.style.left = ev.x +'px';
        selector.el.style.top = ev.y +'px';
        UI.bucket.appendChild(selector.el);
    },
    on_mouse_up: function(ev){
        console.log("mouse up");
        if(selector.down && selector.el && selector.el.parentNode){
        selector.el.parentNode.removeChild(selector.el);
        if(document.getElementById('sel'))document.getElementById('sel').parentNode.removeChild(document.getElementById('sel'));
        selector.down = false;
        var stx = selector.stx;
        var endx = selector.stx+selector.xs*parseInt(selector.el.style.width);
        var sty = selector.sty;
        var endy = selector.sty+selector.ys*parseInt(selector.el.style.height);
        if(stx > endx){
            var tmp = stx;
            stx = endx;
            endx = tmp;
        ...
        for(var a in UI.slices){
            var b = UI.slices[a];
            if(b.x > stx && b.x < endx && b.y > sty && b.y < endy){
                UI.selected.push(b.id);
                $select(b.id);
            }
        }
        for(var a in UI.conts){
    ...
    on_mouse_move: function(ev){
        if(selector.down){
            var w = ev.x - selector.stx;
            var h = ev.y - selector.sty;
            selector.el.style.width = ev.x - selector.stx  +'px';
            selector.el.style.height = ev.y - selector.sty +'px';
            selector.xs = 1;
            selector.ys = 1;
            if(w < 0){
                selector.el.style.width = -w + 'px';
                selector.el.style.left = ev.x + 'px';
                selector.xs = -1;
            }
            if(h < 0){
                selector.el.style.height = -h + 'px';
                selector.el.style.top = ev.y + 'px';
                selector.ys = -1;
            }
        }       
...
function generateAnimation(id,x,y,ex){//generate the animation
	var keyframeprefix = "-webkit-";
	var keyframes = '@' + keyframeprefix + 'keyframes '+id+' { '+'0% {' + keyframeprefix + 'transform:scale(1)}'+'50% {' + keyframeprefix + 'transform:translatex('+x+'px) translatey('+y+'px)}'+'100% {' + keyframeprefix + 'transform:translatex('+ex+'px) translatey('+y+'px)}'+'}';
	keyframes += '#'+id+'{-webkit-animation-duration: 0.5s;-webkit-animation-iteration-count: 1;-webkit-animation-timing-function:linear;}'; 
...
var UI = {
    init: function() {
	    ...
	    //selector
	    UI.selected = Array();
	    UI.bucket.addEventListener('mousedown', selector.on_mouse_down, false);
        UI.bucket.addEventListener('mousemove', selector.on_mouse_move, false);
        document.addEventListener('mouseup',   selector.on_mouse_up, false);
        ...
        UI.slices = Array();
        UI.conts = Array();
        document.body.onclick = $bind(closeAll);
    },
    ...
    delete_selected: function() {
        for(var a in UI.selected){
            var id = UI.selected[a];
            var element = $(UI.selected[a]);
            if (element) {
                ...
                try {
                    ...
                    doc._deleted = true;
                    UI.session.save(doc);
                    if(doc.node.type == 'cont')for(var a in UI.conts){
                        if(UI.conts[a].id == id)UI.conts.splice(a,1);
                    }
                    if(doc.node.type == 'slice')for(var a in UI.slices){
                        if(UI.slices[a].id == id)UI.slices.splice(a,1);
                    }
                }
                catch (e) {
                    return;
    ...
    group_selected: function(){
        var ndoc = create_cont();
        var list = Array();
        if(UI.selected.length < 2)return;
        for(var a in UI.selected){//get all the slices
            var doc = UI.session.get_doc(UI.selected[a]);
            if(doc.node.type == 'slice'){
                for(var b in UI.slices)if(UI.slices[b].id == UI.selected[a]){
                    list.push(UI.slices[b]);
                    UI.slices.splice(b,1);
                }
                //UI.bucket.removeChild(document.getElementById(UI.selected[a]))
                //ndoc.node.src.push(UI.selected[a]);
            }            
        }
        if(list.length < 2)return;
        list.sort(compare);//sort for x position
        var y = 0;
        for(var a in list){//add the slices in the doc and compute the average y
            ndoc.node.src.push(list[a].id);
            y += list[a].y;
        }
        y /= list.length;
        UI.session.save(ndoc);
        var ncont = new Cont(UI.session,ndoc);
        ncont.x = list[0].x;
        ncont.y = y;
        for(var a in list){//animate to form a line
            generateAnimation(list[a].id,list[0].x+(a*130)-list[a].x,y-list[a].y,list[0].x-list[a].x);
            list[a].element.style.webkitAnimationName = list[a].id;
            console.log('animate');
            list[a].element.addEventListener('webkitAnimationEnd',function(ev){
            //console.log('end');
            /*this.style.webkitAnimationName = "";
            this.style.top = "0px";
            this.style.left = "0px";
            $unparent(this);
            ncont.list.appendChild(this);
            ncont.close();*/
            UI.bucket.removeChild(this);
            UI.sequence.do_reorder();
            
            });           
        }
        //setTimeout(UI.sequence.do_reorder(),530);
        //setTimeout(function(){for(var a in list)UI.bucket.removeChild(document.getElementById(list[a].id));},530,list);
        UI.bucket.appendChild(ncont.element);       
    },

    duplicate_selected: function() {
        for(var a in UI.selected){
            var element = $(UI.selected[a]);
            var doc = UI.session.get_doc(element.id);
            if(doc.node.type == 'slice'){
            ...
            if(doc.node.type == 'cont'){
                var ndoc = create_cont();
                ndoc.node.src = doc.node.src;
                UI.session.save(ndoc);
                var slice = new Cont(UI.session, ndoc);               
            }
            slice.x = parseInt(element.style.left)-30;
            slice.y = parseInt(element.style.top)-30;
            UI.bucket.appendChild(slice.element);
        }
        UI.sequence.do_reorder();
    },
    ...
    get_slice: function(_id) {
        ...
        if(doc.node.type == 'cont'){
        var slice = new Cont(UI.session, doc);
        return slice.element;
        }
    },
    ...
    actions: {
        ...
        //G
        'U+0047': function(event){
            UI.group_selected();
        },
        //D
 	    'U+0044': function(event) {
           UI.duplicate_selected();
        },
